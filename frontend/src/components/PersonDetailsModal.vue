  ...
  <Teleport to="body">
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        >
          <div class="absolute -top-6 -end-6 w-32 h-32 rounded-full opacity-20 bg-white"></div>
          <div class="absolute top-4 -start-4 w-20 h-20 rounded-full opacity-10 bg-white"></div>
          ...
          <div v-if="isDeceased" class="absolute top-3 start-3">
            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/20 text-white text-xs font-semibold backdrop-blur-sm">
          ...
          <div v-else class="absolute top-3 start-3">
            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-xs font-semibold backdrop-blur-sm">
        ...
        <!-- Scrollable content -->
        <div class="px-6 pb-6 max-h-[60vh] overflow-y-auto space-y-4">

          <!-- Avatar — overlapping banner -->
          <div class="flex justify-between items-end mt-5">
            <div
              class="w-24 h-24 rounded-2xl overflow-hidden ring-4 shadow-xl flex-shrink-0"
            ...
            <div v-if="age !== null" class="mb-2"
              :class="isDeceased ? 'text-gray-500 dark:text-slate-400' : isMale ? 'text-blue-600 dark:text-blue-400' : 'text-pink-600 dark:text-pink-400'"
            >
          ...
          <div v-if="person.description">
            <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">وصف او ملاحظات</p>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="flex items-start gap-3 p-3 rounded-2xl"
              :class="isDeceased ? 'bg-gray-50 dark:bg-slate-800/50' : isMale ? 'bg-blue-50/60 dark:bg-blue-900/20' : 'bg-pink-50/60 dark:bg-pink-900/20'"
            >
            ...
            >
              ...
              >
                <svg v-if="isDeceased" class="w-4 h-4 text-gray-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          >
            <div class="w-8 h-8 rounded-xl bg-violet-100 dark:bg-violet-800/40 flex items-center justify-center flex-shrink-0">
            ...
            <div>
              <div class="text-xs text-violet-400 dark:text-violet-500 font-medium mb-0.5">{{ $t('person.parent') }}</div>
          ...
          <div>
            ...
            <div v-if="person.spouse"
              class="flex items-center justify-between gap-3 p-3 rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/30"
            >
              <div class="flex items-center gap-3 min-w-0">
                <!-- Avatar -->
                <div
                  class="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 ring-2 ring-amber-200 dark:ring-amber-700"
                  :class="person.spouse.gender === 'male' ? 'bg-blue-50 dark:bg-blue-900/40' : 'bg-pink-50 dark:bg-pink-900/40'"
                >
                  <img v-if="person.spouse.photo_url" :src="person.spouse.photo_url" class="w-full h-full object-cover" />
                  <div v-else class="w-full h-full flex items-center justify-center text-base font-bold"
                    :class="person.spouse.gender === 'male' ? 'text-blue-500' : 'text-pink-500'"
                  >
                    {{ person.spouse.name.charAt(0) }}
                  </div>
                </div>
                <div class="min-w-0">
                  <div class="text-xs text-amber-500 dark:text-amber-400 font-medium mb-0.5 flex items-center gap-1">
                    <span>💍</span>
                    <span>{{ $t('marriage.spouse') }}</span>
                    <span v-if="person.spouse.marriage_date" class="text-amber-400">
                      · {{ new Date(person.spouse.marriage_date).getFullYear() }}
                    </span>
                  </div>
              ...
              <button
                ...
                :title="$t('marriage.unlinkSpouse')"
              >
                <svg v-if="unlinkingSpouse" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
            ...
            <div v-else>
              ...
              >
                <span>💍</span>
                <span>{{ $t('marriage.linkSpouse') }}</span>
                <svg class="w-4 h-4 transition-transform duration-200" :class="showLinkSpouseForm ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
              </button>

              <!-- Link spouse inline form -->
              <Transition
              ...
              >
                <div v-if="showLinkSpouseForm"
                  class="mt-2 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-100 dark:border-amber-800/30 space-y-3"
                >
                  <!-- Person search -->
                  <div>
                    <label class="block text-xs font-semibold text-amber-600 dark:text-amber-400 mb-1.5">
                      {{ $t('marriage.selectSpouse') }}
                    </label>
                    <input
                      v-model="spouseSearch"
                      type="text"
                      :placeholder="$t('marriage.selectSpouse')"
                      class="w-full text-sm px-3 py-2 rounded-xl border border-amber-200 dark:border-amber-700 bg-white dark:bg-slate-800 text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 dark:focus:ring-amber-900/30"
                    />
                    <!-- Filtered list -->
                    <div
                      v-if="filteredSpouseCandidates.length && spouseSearch.trim()"
                      class="mt-1 max-h-36 overflow-y-auto rounded-xl border border-amber-200 dark:border-amber-700 bg-white dark:bg-slate-800 shadow-lg"
                    >
                      <button
                        v-for="p in filteredSpouseCandidates"
                        :key="p.id"
                        type="button"
                        @mousedown.prevent="selectSpouseCandidate(p)"
                        class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-start hover:bg-amber-50 dark:hover:bg-amber-900/30 transition-colors"
                        :class="selectedSpouseId === p.id ? 'bg-amber-100 dark:bg-amber-800/40 font-semibold' : ''"
                      >
                        <span class="text-base">{{ p.gender === 'male' ? '👨' : '👩' }}</span>
                        <span class="text-gray-800 dark:text-slate-200 truncate">{{ p.name }}</span>
                        <span v-if="selectedSpouseId === p.id" class="ms-auto text-amber-500">✓</span>
                      </button>
                    </div>
                    <!-- Selected person chip -->
                    <div v-if="selectedSpouseId && selectedSpouseName"
                      class="mt-2 flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-100 dark:bg-amber-800/40 text-amber-800 dark:text-amber-200 text-sm font-semibold"
                    >
                      <span>💍</span>
                      <span>{{ selectedSpouseName }}</span>
                      <button @click="clearSpouseSelection" class="ms-auto text-amber-500 hover:text-amber-700">
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-amber-600 dark:text-amber-400 mb-1.5">
                      {{ $t('marriage.addSpouseDate') }}
                    </label>
                    <input
                      v-model="marriageDateInput"
                      type="date"
                      class="w-full text-sm px-3 py-2 rounded-xl border border-amber-200 dark:border-amber-700 bg-white dark:bg-slate-800 text-gray-800 dark:text-slate-200 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 dark:focus:ring-amber-900/30"
                    />
                  </div>
                  ...
                  <div class="flex gap-2">
                    <button
                      @click="saveSpouseLink"
                      :disabled="!selectedSpouseId || savingMarriage"
                      class="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 disabled:bg-amber-300 text-white text-sm font-semibold transition-colors"
                    >
                      <svg v-if="savingMarriage" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                      </svg>
                      <span>{{ savingMarriage ? $t('marriage.savingLink') : $t('actions.saveChanges') }}</span>
                    </button>
                    <button
                      @click="cancelLinkSpouse"
                      class="px-4 py-2 rounded-xl bg-white dark:bg-slate-800 border border-amber-200 dark:border-amber-700 text-amber-600 dark:text-amber-400 text-sm font-semibold hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors"
                    >
                      {{ $t('actions.cancel') }}
                    </button>
                  </div>
    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 z-60 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 max-w-xs w-full shadow-2xl">
        <div class="text-center mb-5">
          <div class="w-14 h-14 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/40 flex items-center justify-center mx-auto mb-4">
...
const showLinkSpouseForm  = ref(false)
const spouseSearch        = ref('')
const selectedSpouseId    = ref(null)
const selectedSpouseName  = ref('')
const marriageDateInput   = ref('')
const savingMarriage      = ref(false)
const unlinkingSpouse     = ref(false)
...
// Only show opposite-gender candidates (man → women only, woman → men only)
const oppositeGender = computed(() => props.person.gender === 'male' ? 'female' : 'male')

const filteredSpouseCandidates = computed(() => {
  const q = spouseSearch.value.trim().toLowerCase()
  if (!q) return []
  return props.people
    .filter(p =>
      p.id !== props.person.id &&
      p.gender === oppositeGender.value &&
...
function selectSpouseCandidate(p) {
  selectedSpouseId.value   = p.id
  selectedSpouseName.value = p.name
  spouseSearch.value       = ''
}

function clearSpouseSelection() {
  selectedSpouseId.value   = null
  selectedSpouseName.value = ''
}

function cancelLinkSpouse() {
  showLinkSpouseForm.value = false
  clearSpouseSelection()
  marriageDateInput.value = ''
}

async function saveSpouseLink() {
  if (!selectedSpouseId.value) return
  savingMarriage.value = true
  try {
    await store.addMarriage(
      props.person.id,
      selectedSpouseId.value,
      marriageDateInput.value || null,
    )
    cancelLinkSpouse()
    emit('marriage-changed', props.person.id)
  } catch (e) {
    alert(e?.response?.data?.message ?? 'Failed to link spouse.')
  } finally {
    savingMarriage.value = false
  }
}

async function unlinkSpouse() {
  if (!props.person.spouse?.marriage_id) return
  unlinkingSpouse.value = true
  try {
    await store.deleteMarriage(props.person.spouse.marriage_id)
    emit('marriage-changed', props.person.id)
  } catch (e) {
    alert(e?.response?.data?.message ?? 'Failed to unlink spouse.')
  } finally {
    unlinkingSpouse.value = false
  }
}
</script>
