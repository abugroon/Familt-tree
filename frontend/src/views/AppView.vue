  ...
  <div class="min-h-screen bg-gray-50 dark:bg-slate-950">
    ...
    <header class="sticky top-0 z-40 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700/60 shadow-sm">
      <div class="max-w-screen-2xl mx-auto px-4">
        ...
        <!-- Top row: Logo | (desktop search) | Controls -->
        <div class="flex items-center gap-2 py-3">
          ...
          <div class="flex items-center gap-2.5 shrink-0">
            ...
            <h1 class="text-base font-bold text-gray-900 dark:text-white hidden sm:block">{{ $t('app.title') }}</h1>
          </div>

          <!-- Search — desktop only (md+) -->
          <div class="hidden md:block flex-1 relative max-w-sm" ref="desktopSearchRef">
            <div class="relative">
              ...
              <input
                  ...
                  class="w-full ps-9 pe-3 py-2 rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-sm text-gray-800 dark:text-slate-200 placeholder-gray-400 focus:outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 dark:focus:ring-violet-900/30"
              />
            </div>
            <div
                v-if="searchOpen && searchResults.length"
                class="absolute top-full mt-1.5 start-0 end-0 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden z-50 max-h-64 overflow-y-auto"
            >
              <button
                  v-for="person in searchResults" :key="person.id"
                  type="button"
                  @mousedown.prevent="onSearchSelect(person)"
                  class="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-700 text-start transition-colors"
              >
                <div
                    class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 overflow-hidden"
                    :class="person.gender === 'male' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-600' : 'bg-pink-100 dark:bg-pink-900/50 text-pink-600'"
                >
                  <img v-if="person.photo_url" :src="person.photo_url" class="w-full h-full object-cover" />
                  <span v-else>{{ person.name.charAt(0) }}</span>
                </div>
                <span class="text-sm text-gray-800 dark:text-slate-200 truncate">{{ person.name }}</span>
                <span class="text-xs ms-auto shrink-0">{{ person.gender === 'male' ? '👨' : '👩' }}</span>
              </button>
            </div>
          </div>

          <!-- Spacer (mobile only — pushes controls to right) -->
          ...
          <div class="flex items-center gap-1.5 shrink-0">
            ...
            <button @click="toggleLocale"
                    class="w-9 h-9 rounded-xl flex items-center justify-center border transition-all active:scale-95 bg-gray-100 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700 text-xs font-bold"
            ...
            <button
                ...
                class="w-9 h-9 rounded-xl flex items-center justify-center border transition-all active:scale-95 bg-gray-100 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-slate-700"
                title="تصدير كصورة">
              <svg v-if="familyTreeRef?.exporting" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
            ...
            <button
                ...
                @click="openAddModal()"
                class="flex items-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white text-sm font-semibold shadow-md shadow-violet-200 dark:shadow-violet-900/40 active:scale-95"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4" />
              </svg>
              <span class="hidden sm:inline">{{ $t('actions.addPersonTitle') }}</span>
            </button>
            ...
            <button
                ...
                class="flex items-center gap-2 px-3 py-2 rounded-xl border transition-all active:scale-95 bg-gray-100 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700 text-sm font-semibold"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-if="authStore.isAuthenticated" class="relative" ref="userDropdownRef">
              <button
        ...
        <!-- Search row — mobile only (below md) -->
        <div class="md:hidden pb-3 relative" ref="searchRef">
          <div class="relative">
            <svg class="absolute start-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
            </svg>
          ...
          <div
              v-if="searchOpen && searchResults.length"
              class="absolute top-full mt-0 start-0 end-0 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden z-50 max-h-64 overflow-y-auto"
          >
            <button
                v-for="person in searchResults" :key="person.id"
                type="button"
    ...
    <main class="relative">
      ...
      <!-- Empty state — show add button for admin only -->
      ...
      <!-- Tree View -->
      <div v-else class="tree-canvas-height">
        <FamilyTree
            ref="familyTreeRef"
            :trees="store.trees"
    ...
    <!-- Floating Add Button (mobile) — admin only -->
    <button
        v-if="store.trees.length && authStore.isAdmin"
        @click="openAddModal()"
...
const { showAddModal, addChildParentId, openAddModal, closeAddModal } = useAddModal()
...
const searchResults = computed(() => {
  ...
  if (!q) return []
  return store.allPeople.filter(p => p.name.toLowerCase().includes(q)).slice(0, 8)
})
...
function handleClickOutside(e) {
  ...
  const inSearch = (searchRef.value?.contains(e.target)) || (desktopSearchRef.value?.contains(e.target))
  if (!inSearch) searchOpen.value = false
}
...
<style scoped>
.tree-canvas-height { height: calc(100dvh - 65px); }

@media (max-width: 767px) {
  .tree-canvas-height { height: calc(100dvh - 107px); }
}
</style>
